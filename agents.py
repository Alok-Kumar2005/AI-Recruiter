...
class ResumeAnalysisAgent:
    ...
    def create_rag_vector_store(self, text):
        ...
        embeddings = GoogleGenerativeAIEmbeddings(model="models/text-embedding-004" , api_key=self.api_key)
    ...
    def analyze_resume_weaknesses(self):
        ...
        for skill in self.analysis_result.get("missing_skills", []):
            ...
            llm = ChatGoogleGenerativeAI(model= "gemini-1.5-pro", api_key=self.api_key)
            prompt = f"""
    ...
    def semantic_skill_analysis(self, resume_text, skills):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:  ## these will divide the work in 5 different steps and each of them will work independently
    ...
    ## answering question's on the basis of 
